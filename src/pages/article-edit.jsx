...
export default function ArticleEdit() {
  ...
  const Record  = async () => {
      try{
          ...
          const result = await axios.patch(`https://q27z6n.deta.dev/articles/${id}`, JSON.stringify(postBody), { headers: { 'x-token': token, 'Content-Type': 'application/json' } });
  ...
  useEffect(() => {
    const FetchData = async () => {
      const result = await axios.get(`https://q27z6n.deta.dev/articles/${id}`);
      console.log(result)
      setContents(Base64.decode(result.data.content))
      setTitle(result.data.title)
      setimgHeader(result.data.image)
    }
      try{
        FetchData()
      }catch(error){
  ...
  <div id="main_template">
    ...
    <button onClick={() => {navigator.clipboard.writeText(JSON.stringify(returnBody()))}}>copy</button>
        ...
        <SunEditor autoFocus={false} onChange={setData} width="100%" height='100%'
